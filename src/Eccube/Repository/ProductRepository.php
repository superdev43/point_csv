...
class ProductRepository extends EntityRepository
{
    ...
    public function getQueryBuilderBySearchData($searchData)
    {
        ...
        if (!empty($searchData['orderby']) && $searchData['orderby']->getId() == $config['product_order_price_lower']) {
            ...
            // ▼更新順
        } else if (!empty($searchData['orderby']) && $searchData['orderby']->getId() == 999) {
            if ($this->app['orm.em']->getFilters()->isEnabled('nostock_hidden') == true) {
                $qb->innerJoin('p.ProductClasses', 'pc');
            }
            $qb->orderBy('p.list_update', 'DESC');
    ...
    public function getQueryBuilderBySearchDataForAdmin($searchData)
    {
       ...
       */
        ...
        // ▼更新順設定用の日時
        if (!empty($searchData['list_update_start']) && $searchData['list_update_start']) {
            $date = $searchData['list_update_start']
                ->format('Y-m-d H:i:s');
            $qb
                ->andWhere('p.list_update >= :list_update_start')
                ->setParameter('list_update_start', $date);
        }
        if (!empty($searchData['list_update_end']) && $searchData['list_update_end']) {
            $date = clone $searchData['list_update_end'];
            $date = $date
                ->modify('+1 days')
                ->format('Y-m-d H:i:s');
            $qb
                ->andWhere('p.list_update < :list_update_end')
                ->setParameter('list_update_end', $date);
        }


        // Order By
        $qb
            // ▼更新順設定用の日時（管理画面の検索結果の並び順）
			//->orderBy('p.update_date', 'DESC');
			->orderBy('p.list_update', 'DESC');
			$qb->addOrderBy('p.update_date', 'DESC');
