...
class PlgExpandProductColumnsCsvImportController extends \Eccube\Controller\Admin\Product\CsvImportController
{
    ...
    private $ex_column_headers = array();

    /**
     * 商品登録CSVヘッダー定義
     */
    private function getProductCsvHeader($ex_column_headers = array())
    {
        $headers = array(
            '商品ID' => 'id',
            '公開ステータス(ID)' => 'status',
        ...
        foreach ($ex_column_headers as $header) {
            $headers[$header['name']] = $header['id'];
        }

        return $headers;
    }

    static function getExColumnHeaders(Application $app)
    {
        $headers = array();
        ...
        foreach ($entities as $entity) {
            $headers[$entity->getColumnId()] = array(
                'id' => $entity->getColumnId(),
                'name' => $entity->getColumnName(),
                'type' => $entity->getColumnType(),
                'description' => self::getDescription($entity->getColumnType())
            );
        }
    ...
    static function getDescription($column_type)
    {
        $result = '';
        switch ($column_type) {
            case EX_TYPE_CHECKBOX :
                $result = '複数件ある場合はカンマ区切りで設定。';
                break;
            case EX_TYPE_IMAGE :
                $result = '画像ファイル名を設定。<br>複数件ある場合はカンマ区切りで設定。';
                break;
    ...
    public function csvProduct(Application $app, Request $request)
    {
        ...
        $this->ex_column_headers = self::getExColumnHeaders($app);
        $headers = $this->getProductCsvHeader($this->ex_column_headers);
            ...
            if ($form->isValid()) {
                ...
                if (!empty($formFile)) {
                    ...
                    foreach ($data as $row) {
                         ...
                         * 拡張項目登録
                         */
                        foreach ($this->ex_column_headers as $column_info) {
                            $exValue = $app['eccube.plugin.repository.plg_expand_product_columns_value']->findOneBy(array(
                                'productId' => $row['商品ID'],
                                'columnId' => $column_info['id']
                            ));
                            ...
                            if (!$exValue) {
                                 ...
                                 * なければ新規追加
                                 */
                                $exValue = new PlgExpandProductColumnsValue();
                                $exValue->setProductId($Product->getId());
                                $exValue->setColumnId($column_info['id']);
                                $this->em->persist($exValue);
                            }
                            $exValue->setValue($row[$column_info['name']]);
                            $this->em->flush();
                        }
    ...
    public function csvTemplate(Application $app, Request $request, $type)
    {
        ...
        if ($type == 'product') {
            $headers = $this->getProductCsvHeader(self::getExColumnHeaders($app));
    ...
    /****************************************************************************************************************
     *  以下は特に手を加えていないが、privateなメンバ変数を利用していて、且つコンストラクタによるセットではないので
     *  本クラスで定義しないと不具合を起こすと思われるので、下記にコピペ
     ***************************************************************************************************************/
