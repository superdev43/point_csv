...
namespace Plugin\GmoEpsilon\Controller\Admin\Order;
...
class RegularEditController extends AbstractController
{
    public function index(Application $app, Request $request, $id)
    {
                    ...
                    if ($TargetRegularOrder->getTotal() > $app['config']['max_total_fee']) {
                        ...
                        // 複数配送が無効
                            ...
                            foreach ($TargetRegularOrder->getRegularOrderDetails() as $RegularOrderDetail) {
                                ...
                                $NewRegularShipmentItem = new \Plugin\GmoEpsilon\Entity\RegularShipmentItem();
                        ...
                        return $app->redirect($app->url('epsilon_regular_order_edit', array('id' => $TargetRegularOrder->getId())));
                    }
                    break;
                case 'add_delivery':
                    // お届け先情報の新規追加
                    $form = $builder->getForm();
                    $RegularShipping = new \Plugin\GmoEpsilon\Entity\RegularShipping();
        ...
        return $app->render('GmoEpsilon/Twig/admin/Order/regular_edit.twig', array(
    ...
    public function searchProduct(Application $app, Request $request)
    {
        if ($request->isXmlHttpRequest()) {
            ...
            return $app->render('GmoEpsilon/Twig/admin/Order/search_regular_product.twig', array(
                'forms' => $forms,
                'Products' => $Products,
            ));
        }
     ...
     * @param $Order
     */
    protected function calculate($app, \Plugin\GmoEpsilon\Entity\RegularOrder $RegularOrder)
    {
        $taxtotal = 0;
        $subtotal = 0;
