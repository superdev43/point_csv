...
use Plugin\GmoEpsilon\Form\Type\Admin\ConfigType;
...
class ConfigController
{
    ...
    public function index(Application $app, Request $request)
    {
        ...
        $sslVer_names = $objPayUtil->getSSLVersionName();
        $sslVer_numbers = $objPayUtil->getSSLVersionNumber();
        ...
        if ('POST' === $this->app['request']->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                // ssl対応判定
                if (!extension_loaded('openssl') && preg_match("/^https:/", $formData['destination_url'])) {
                    $form['destination_url']->addError(new FormError('※ このサーバはSSLに対応していません。httpで接続してください。'));
                }
                ...
                $arrXML = $this->app['eccube.plugin.epsilon.service.base']->sendData($formData['destination_url'], $arrParameter);
                if ($arrXML === false) {
                    $form['destination_url']->addError(new FormError('※ 接続できませんでした。'));
                } else if ($arrXML == $this->const['SSLVERSION_ERROR_CODE']) {
                ...
                $arrXML = $this->app['eccube.plugin.epsilon.service.base']->sendData($formData['info_conf_url'], array('contarct_code' => $formData['contract_code']));
                if ($arrXML === false) {
                    $form['info_conf_url']->addError(new FormError('※ 接続できませんでした。'));
                } else if ($arrXML == $this->const['SSLVERSION_ERROR_CODE'] && $form['ssl_version']->isValid() === true) {
                	$form['ssl_version']->addError(new FormError('※ご指定のSSLバージョンには対応していません'));
                }
                ...
                if ($form->isValid()) {
                    ...
                    return $this->app->redirect($this->app['url_generator']->generate('plugin_GmoEpsilon_config'));
                }
            }
        }

        return $this->app['view']->render('GmoEpsilon/Twig/admin/config.twig',
            array(
                'form' => $form->createView(),
                'tpl_subtitle' => $tpl_subtitle,
                'subData' => $subData,
            	'sslVer_names' => $sslVer_names,
            ));
    }
    ...
    /**
     * plg_epsilon_payment_extensionに決済情報を登録
     *
     * @param integer $id
     * @param integer $paymentTypeId
     * @param array $Payments
