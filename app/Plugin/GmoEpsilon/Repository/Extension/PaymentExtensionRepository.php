...
class PaymentExtensionRepository extends EntityRepository
{
    ...
    {
        if ($id == 0) {
            $PaymentExtension = new \Plugin\GmoEpsilon\Entity\Extension\PaymentExtension();
        } else {
    ...
    /**
     * Get all payment_id of table dtb_payment base on plugin_code of table plg_epsilon_payment_extension
     *
    ...
    public function getPaymentByCode($incDel, $app)
    {
        ...
        if ($incDel){
            $softDeleteFilter->setExcludes(array(
                ...
                'Plugin\GmoEpsilon\Entity\Extension\PaymentExtension'
            ));
        }
        ...
        $qb
            ...
            ->innerJoin('\Plugin\GmoEpsilon\Entity\Extension\PaymentExtension', 'g', 'WITH', 'p.id = g.id')
            ->where($qb->expr()->andx(
    ...
    public function getPaymentByType($paymentTypeId, $incDel, $app)
    {
        ...
        $qb
            ->select('p')
            ->from('\Eccube\Entity\Payment', 'p')
            ->join('\Plugin\GmoEpsilon\Entity\Extension\PaymentExtension', 'g', 'WITH', 'p.id = g.id')
            ->where(
